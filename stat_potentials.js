/**
 * stat_potentials.js — Stat Potentials Table (RM2 Character Law Table 15.1.1 / 05-01)
 * 
 * VERIFIED against CPR093.exe screenshots (7/7 cases match perfectly).
 * 
 * Algorithm used by CPR093:
 * 1. For each of the 10 stats, roll temp_roll (1-100) and pot_roll (1-100)
 * 2. temp = temp_roll
 * 3. pot = statPotentialLookup(pot_roll, temp)
 * 4. Display as "temp(pot)" in the roll list
 * 5. For prime stats (class primary characteristics):
 *    a. new_temp = max(temp_roll, 90)
 *    b. new_pot = statPotentialLookup(pot_roll, new_temp)  // recalculated with boosted temp
 *    c. new_pot = max(new_pot, new_temp)  // pot can never be < temp (safety)
 * 
 * IMPORTANT: CPR093 stores the RAW pot_roll internally, not the displayed pot.
 * When a roll is assigned to a prime stat, the pot is recalculated from the raw pot_roll
 * using the new (boosted) temp as the column lookup.
 * 
 * This can sometimes result in a LOWER displayed pot after prime boosting,
 * which is correct RM2 behavior (not a bug).
 */

const STAT_POTENTIAL_TABLE = {
  // Column boundaries: [min_temp, max_temp]
  columns: [
    [1, 24],    // 0: Under 25
    [25, 39],   // 1: 25-39
    [40, 59],   // 2: 40-59
    [60, 74],   // 3: 60-74
    [75, 84],   // 4: 75-84
    [85, 89],   // 5: 85-89
    [90, 94],   // 6: 90-94
    [95, 97],   // 7: 95-97
    [98, 99],   // 8: 98-99
    [100, 100]  // 9: 100
  ],

  // Each row: [roll_min, roll_max, [values per column]]
  // null = "-" = potential equals the initial stat
  rows: [
    [1,  10,  [25,   null, null, null, null, null, null, null, null, null]],
    [11, 20,  [30,   null, null, null, null, null, null, null, null, null]],
    [21, 30,  [35,   39,   null, null, null, null, null, null, null, null]],
    [31, 35,  [38,   42,   59,   null, null, null, null, null, null, null]],
    [36, 40,  [40,   45,   62,   null, null, null, null, null, null, null]],
    [41, 45,  [42,   47,   64,   null, null, null, null, null, null, null]],
    [46, 49,  [44,   49,   66,   null, null, null, null, null, null, null]],
    [50, 51,  [46,   51,   68,   null, null, null, null, null, null, null]],
    [52, 53,  [48,   53,   70,   null, null, null, null, null, null, null]],
    [54, 55,  [50,   55,   71,   null, null, null, null, null, null, null]],
    [56, 57,  [52,   57,   72,   74,   84,   null, null, null, null, null]],
    [58, 59,  [54,   59,   73,   75,   85,   null, null, null, null, null]],
    [60, 61,  [56,   61,   74,   76,   86,   null, null, null, null, null]],
    [62, 63,  [58,   63,   75,   77,   87,   null, null, null, null, null]],
    [64, 65,  [60,   65,   76,   78,   88,   null, null, null, null, null]],
    [66, 67,  [62,   67,   77,   79,   88,   89,   null, null, null, null]],
    [68, 69,  [64,   69,   78,   80,   89,   89,   null, null, null, null]],
    [70, 71,  [66,   71,   79,   81,   89,   90,   null, null, null, null]],
    [72, 73,  [68,   73,   80,   82,   90,   90,   null, null, null, null]],
    [74, 75,  [70,   75,   81,   83,   90,   91,   null, null, null, null]],
    [76, 77,  [72,   77,   82,   84,   91,   91,   null, null, null, null]],
    [78, 79,  [74,   79,   83,   85,   91,   92,   null, null, null, null]],
    [80, 81,  [76,   81,   84,   86,   92,   92,   null, null, null, null]],
    [82, 83,  [78,   83,   85,   87,   92,   93,   null, null, null, null]],
    [84, 85,  [80,   85,   86,   88,   93,   93,   94,   null, null, null]],
    [86, 87,  [82,   86,   87,   89,   93,   94,   94,   null, null, null]],
    [88, 89,  [84,   87,   88,   90,   94,   94,   95,   null, null, null]],
    [90, 90,  [86,   88,   89,   91,   94,   95,   95,   97,   null, null]],
    [91, 91,  [88,   89,   90,   92,   95,   95,   96,   97,   null, null]],
    [92, 92,  [90,   90,   91,   93,   95,   96,   96,   97,   null, null]],
    [93, 93,  [91,   91,   92,   94,   96,   96,   97,   98,   null, null]],
    [94, 94,  [92,   92,   93,   95,   96,   97,   97,   98,   99,   null]],
    [95, 95,  [93,   93,   94,   96,   97,   97,   98,   98,   99,   null]],
    [96, 96,  [94,   94,   95,   97,   97,   98,   98,   99,   99,   null]],
    [97, 97,  [95,   95,   96,   97,   98,   98,   99,   99,   99,   null]],
    [98, 98,  [96,   96,   97,   98,   98,   99,   99,   99,   100,  null]],
    [99, 99,  [97,   97,   98,   98,   99,   99,   100,  100,  100,  null]],
    [100,100, [98,   98,   99,   99,   99,   100,  100,  100,  100,  101]]
  ]
};

/**
 * Get the column index for a given temp stat value.
 * @param {number} temp - The temporary stat value (1-101)
 * @returns {number} Column index (0-9)
 */
function getColumnIndex(temp) {
  for (let i = 0; i < STAT_POTENTIAL_TABLE.columns.length; i++) {
    const [min, max] = STAT_POTENTIAL_TABLE.columns[i];
    if (temp >= min && temp <= max) return i;
  }
  // Fallback: if temp > 100, use column 9
  if (temp > 100) return 9;
  return 0; // if temp < 1, use column 0
}

/**
 * Look up the stat potential from the table.
 * @param {number} potRoll - The raw potential roll (1-100)
 * @param {number} temp - The temporary stat value
 * @returns {number} The calculated potential
 */
function statPotentialLookup(potRoll, temp) {
  const colIdx = getColumnIndex(temp);
  
  for (const row of STAT_POTENTIAL_TABLE.rows) {
    if (potRoll >= row[0] && potRoll <= row[1]) {
      const val = row[2][colIdx];
      return val === null ? temp : val;
    }
  }
  // Should never reach here for valid rolls 1-100
  return temp;
}

/**
 * Generate a complete set of 10 stat rolls.
 * Each stat gets a temp_roll and a pot_roll.
 * Returns the displayed values (after table lookup) AND the raw rolls (for prime recalculation).
 * 
 * @returns {Array<{tempRoll: number, potRoll: number, temp: number, pot: number}>}
 */
function generateStatRolls() {
  const rolls = [];
  for (let i = 0; i < 10; i++) {
    const tempRoll = Math.floor(Math.random() * 100) + 1;
    const potRoll = Math.floor(Math.random() * 100) + 1;
    const temp = tempRoll;
    const pot = statPotentialLookup(potRoll, temp);
    rolls.push({ tempRoll, potRoll, temp, pot });
  }
  return rolls;
}

/**
 * Apply prime stat bonus when a roll is assigned to a primary characteristic.
 * The temp is boosted to at least 90, and the pot is recalculated via the table
 * using the boosted temp.
 * 
 * @param {{tempRoll: number, potRoll: number, temp: number, pot: number}} roll
 * @returns {{temp: number, pot: number}} The boosted temp and pot
 */
function applyPrimeBonus(roll) {
  const newTemp = Math.max(roll.tempRoll, 90);
  const newPot = statPotentialLookup(roll.potRoll, newTemp);
  return {
    temp: newTemp,
    pot: Math.max(newPot, newTemp) // pot should never be less than temp
  };
}

/**
 * Get the displayed values for a roll assigned to a stat.
 * @param {{tempRoll: number, potRoll: number}} roll - The raw roll data
 * @param {boolean} isPrime - Whether this stat is a primary characteristic
 * @returns {{temp: number, pot: number}}
 */
function getStatValues(roll, isPrime) {
  if (isPrime) {
    return applyPrimeBonus(roll);
  }
  return {
    temp: roll.tempRoll,
    pot: statPotentialLookup(roll.potRoll, roll.tempRoll)
  };
}

// Export for use in PWA engine
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    STAT_POTENTIAL_TABLE,
    getColumnIndex,
    statPotentialLookup,
    generateStatRolls,
    applyPrimeBonus,
    getStatValues
  };
}
