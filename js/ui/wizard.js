...
import { logEvent, logStatRoll, logStatValidate, logSkillDevelop, logSpellSGR,
...
import { getBackgroundBonuses, resolveBackgroundChoice, resolveStatIncrease, removeBackgroundOption, summarizeBackgroundBonuses, generateWealthText, applyBgCostModifiers } from '../engine/background-effects.js';
...
import { getWeaponPriorityOrder, WEAPON_TYPE_MAP as WEAPON_TYPE_MAP_ENGINE, autoSpendDP, spendWeaponSkillDP, detectArchetype, autoAddWeaponSkills } from '../engine/npc-generator.js';
...
function renderEditor(app) {
  ...
  const gmBanner = isGMEditMode()
    ? `<div class="gm-edit-banner">🎭 ${app.lang === 'en' ? 'GM Edit Mode — all fields editable' : 'Mode MJ — tous les champs sont modifiables'}</div>`
    : '';
...
function renderStatsTab(lang) {
  ...
  const bgStatMods = getBackgroundBonuses(character).statBonusMods;
  ...
  for (let i = 0; i < STAT_COUNT; i++) {
    ...
    const bg = bgStatMods[i] || 0;
    const total = norm + race + spec + bg;
    ...
    table += `
      <tr class="stat-row ${rowClickable ? 'stat-row-clickable' : ''} ${hasAssignment ? 'stat-row-assigned' : ''}" data-stat-idx="${i}">
        ...
        <td class="text-center stat-bonus ${bg > 0 ? 'positive' : bg < 0 ? 'negative' : 'zero'}">${bg !== 0 ? (bg >= 0 ? '+' + bg : bg) : '0'}</td>
  ...
  document.querySelectorAll('.race-row').forEach(row => {
    row.addEventListener('click', () => {
      if (character.bgOptionsLocked && !isGMEditMode()) {
        showToast(app.lang === 'en' ? 'Race is locked after apprenti validation' : 'Race verrouillée après la validation apprenti', true);
        return;
      }
  ...
  if (character.spellLists.length > 0) {
    ...
    character.spellLists.forEach((sl, i) => {
      ...
      learnedRows += `<tr class="${isStudying ? 'text-purple-300' : ''}">
        ...
        <td class="text-center text-xs text-gray-500">${sl.dpPerRank || rankCost}</td>
  ...
  for (const rg of sortedGroups) {
    ...
    for (const realm of matchingRealms) {
      ...
      if (items) {
        ...
        const sampleCostInfo = getSpellListCost(cls, classRealm, '', realm.name);
        const realmRankCost = sampleCostInfo.cost;
        const costHint = `<span class="text-gray-600 font-normal ml-2">[${realmRankCost} PD/${lang === 'en' ? 'rk' : 'rg'}, ${lang === 'en' ? 'block' : 'bloc'} ${subBlockSize} ${lang === 'en' ? 'lvl' : 'niv'}]</span>`;
...
// Helper: build list of { name, globalIndex } filtered by type (primary/secondary)
function getSkillChoices(character, skillType) {
  const result = [];
  let gi = 0;
  for (const cat of getAllCategories()) {
    for (const skill of cat.skills) {
      const cost = getSkillDevCost(character.classIndex, gi);
      if (cost) {
        const isPrimary = cost.first <= 3;
        const wantPrimary = skillType === 'primary';
        const wantSecondary = skillType === 'secondary';
        if ((wantPrimary && isPrimary) || (wantSecondary && !isPrimary) || (!wantPrimary && !wantSecondary))
          result.push({ name: skill.name_fr || skill.name_en || skill.name, globalIndex: gi });
      }
      gi++;
    }
  }
...
// Render inline choice UI for bg options that require a player choice
function renderBgChoiceUI(opt, idx, lang) {
  const eff = opt.effects || {};
  const resolved = opt.resolved || {};

  // Stat choice: +X to one chosen stat
  if (eff.stat_bonus && typeof eff.stat_bonus === 'object' && 'choice' in eff.stat_bonus) {
    const physical = ['CO','AG','AD','FO','RP'];
    const mental   = ['ME','RS','PR','EM','IN'];
    const allowed  = (opt.id || '').startsWith('rmc1-arms') ? physical : mental;
    return `<select class="bg-choice-select field" data-opt="${idx}" data-type="stat_choice" style="font-size:0.75rem;margin-top:4px">
      <option value="">— ${lang === 'en' ? 'Choose stat' : 'Choisir stat'} —</option>
      ${allowed.map(s => `<option value="${s}" ${resolved.stat_choice === s ? 'selected' : ''}>${s}</option>`).join('')}
    </select>`;
  }

  // Skill choice: +X to one chosen skill (value = skill name for misc bonus matching)
  if (eff.skill_bonus !== undefined && typeof eff.skill_bonus === 'number') {
    const skills = getSkillChoices(character, eff.skill_type);
    return `<select class="bg-choice-select field" data-opt="${idx}" data-type="skill_choice" style="font-size:0.75rem;margin-top:4px">
      <option value="">— ${lang === 'en' ? 'Choose skill' : 'Choisir compétence'} —</option>
      ${skills.map(s => `<option value="${s.name}" ${resolved.skill_choice === s.name ? 'selected' : ''}>${s.name}</option>`).join('')}
    </select>`;
  }

  // Skill ranks: +N ranks to one chosen skill (value = globalIndex)
  if (eff.skill_ranks !== undefined) {
    const skills = getSkillChoices(character, eff.skill_type);
    return `<select class="bg-choice-select field" data-opt="${idx}" data-type="skill_ranks_target" style="font-size:0.75rem;margin-top:4px">
      <option value="">— ${lang === 'en' ? 'Choose target skill' : 'Choisir compétence cible'} —</option>
      ${skills.map(s => `<option value="${s.globalIndex}" ${resolved.skill_ranks_target === s.globalIndex ? 'selected' : ''}>${s.name}</option>`).join('')}
    </select>`;
  }

  // Skill cost override: pick N skills for halved cost (value = array of globalIndex)
  if (eff.skill_cost_multiplier_choice !== undefined) {
    const count = eff.skill_choice_count || 1;
    const skills = getSkillChoices(character, null);
    const targets = resolved.skill_cost_targets || [];
    const selects = Array.from({ length: count }, (_, slot) =>
      `<select class="bg-cost-target-select field" data-opt="${idx}" data-slot="${slot}" style="font-size:0.75rem;margin-top:2px">
        <option value="">— ${lang === 'en' ? 'Skill' : 'Compétence'} ${slot + 1} —</option>
        ${skills.map(s => `<option value="${s.globalIndex}" ${targets[slot] === s.globalIndex ? 'selected' : ''}>${s.name}</option>`).join('')}
      </select>`
    ).join('');
    return selects;
  }
  ...
  if (eff.language_rank) {
    return `<input class="bg-choice-input field" data-opt="${idx}" data-type="language_name"
      placeholder="${lang === 'en' ? 'Language…' : 'Langue…'}" value="${resolved.language_name || ''}"
      style="font-size:0.75rem;margin-top:4px;width:100%">`;
  }
...
// === Event log rendering ===
const EVENT_TYPE_MAP_LOG = {
  stat_roll: 'stats', stat_validate: 'stats', stat_manual_edit: 'stats',
  hp_roll: 'stats', level_up: 'stats', phase_validate: 'stats', stat_gain: 'stats',
  skill_develop: 'skills', skill_manual_edit: 'skills',
  spell_sgr: 'spells', spell_list_add: 'spells', spell_list_remove: 'spells',
  spell_learn: 'spells', spell_manual: 'spells',
  bg_option_roll: 'notes', bg_option_add: 'notes', bg_option_remove: 'notes',
  bg_option_resolve: 'notes', bg_options_locked: 'notes', note: 'notes',
};
const EVENT_LABELS_LOG = {
  stat_roll: { fr: 'Jet de stat', en: 'Stat roll' },
  stat_validate: { fr: 'Stats validées', en: 'Stats validated' },
  stat_manual_edit: { fr: 'Stat éditée', en: 'Stat edited' },
  skill_develop: { fr: 'Compétence développée', en: 'Skill developed' },
  skill_manual_edit: { fr: 'Bonus manuel', en: 'Manual bonus' },
  spell_sgr: { fr: 'Jet SGR', en: 'SGR roll' },
  spell_list_add: { fr: 'Liste ajoutée', en: 'List added' },
  spell_list_remove: { fr: 'Liste retirée', en: 'List removed' },
  spell_learn: { fr: 'Sort appris', en: 'Spell learned' },
  spell_manual: { fr: 'Sort octroyé', en: 'Spell granted' },
  phase_validate: { fr: 'Phase validée', en: 'Phase validated' },
  level_up: { fr: 'Passage de niveau', en: 'Level up' },
  stat_gain: { fr: 'Gains de niveau', en: 'Level gains' },
  bg_option_roll: { fr: 'Option d\'historique', en: 'Background option' },
  bg_option_add: { fr: 'Option ajoutée', en: 'Option added' },
  bg_option_remove: { fr: 'Option retirée', en: 'Option removed' },
  bg_option_resolve: { fr: 'Choix résolu', en: 'Choice resolved' },
  bg_options_locked: { fr: 'Options verrouillées', en: 'Options locked' },
  hp_roll: { fr: 'Jet PdC', en: 'HP roll' },
  note: { fr: 'Note', en: 'Note' },
};

function renderEventLogHTML(events, lang) {
  if (!events || events.length === 0)
    return `<div style="color:#6b5030;font-size:0.82rem;padding:0.75rem">${lang === 'en' ? 'No events yet.' : 'Aucun événement.'}</div>`;
  return [...events].reverse().map(e => {
    const type = EVENT_TYPE_MAP_LOG[e.type] || 'notes';
    const d = new Date(e.timestamp);
    const time = d.toLocaleDateString(lang === 'en' ? 'en' : 'fr') + ' · ' +
      d.toLocaleTimeString(lang === 'en' ? 'en' : 'fr', { hour: '2-digit', minute: '2-digit' });
    const title = EVENT_LABELS_LOG[e.type]?.[lang] || e.type;
    const tag = { stats: 'Stats', skills: lang === 'en' ? 'Skills' : 'Compét.', spells: lang === 'en' ? 'Spells' : 'Sorts', notes: lang === 'en' ? 'Notes' : 'Notes' }[type] || type;
    return `<article class="rm-timeline-item" data-type="${type}">
      ...
      ${e.summary ? `<div class="rm-timeline-body" style="font-size:0.82rem;margin-top:0.4rem">${esc(e.summary)}</div>` : ''}
    </article>`;
  }).join('');
}
...
function renderHistoryTab(lang) {
  ...
  const bgLocked = character.bgOptionsLocked === true && !isGMEditMode();
  let optionsHtml = bgLocked
    ? `<div class="text-xs text-amber-400 mb-2" style="padding:4px 8px;border:1px solid rgba(251,191,36,0.3);border-radius:4px">🔒 ${lang === 'en' ? 'Background options locked since level 1' : 'Options verrouillées depuis le niveau 1'}</div>`
    : '';
      ...
      optionsHtml += `<div class="panel" style="padding:0.5rem 0.75rem;margin-bottom:0.5rem">
        ...
        ${(() => {
          ...
          if (isResolved || !opt.requires_choice) {
            ...
            // Locked: show unresolved choice as read-only warning
            return `<div class="text-xs text-amber-400 mt-1">⚠ ${lang === 'en' ? 'Unresolved choice (locked)' : 'Choix non résolu (verrouillé)'}</div>`;
          } else {
            const inlineUI = renderBgChoiceUI(opt, i, lang);
            return inlineUI
              ? `<div class="text-xs text-amber-400 mt-1">⚠ ${lang === 'en' ? 'Choice required' : 'Choix requis'}${inlineUI}</div>`
              : `<div class="text-xs text-amber-400 mt-1">⚠ ${lang === 'en' ? 'Choice required' : 'Choix requis'} <button class="btn-primary text-xs bg-opt-resolve" data-bg-idx="${i}" style="padding:1px 6px;margin-left:4px">${lang === 'en' ? 'Choose' : 'Choisir'}</button></div>`;
          }
        })()}
        ${bgLocked ? '' : `<button class="text-red-400 text-xs mt-1 bg-opt-remove" data-bg-idx="${i}">✕ ${lang === 'en' ? 'Remove' : 'Retirer'}</button>`}
      </div>`;
    } else if (!bgLocked) {
      // Empty slot — show category selector (only if not locked)
    ...
    ${panel(lang === 'en' ? 'Event Journal' : 'Journal des événements', `
      <div class="rm-journal-shell">
        ...
        <div class="rm-timeline" id="event-journal-container" style="max-height:320px;overflow-y:auto">
          ${renderEventLogHTML(character.eventLog || [], lang)}
        </div>
      </div>
    `)}
  `;
...
/**
 * Builds a Map of computed skill values from getComputedSkills().
 * Key: globalIndex (number) for normal skills,
 *      'wpn_N' (string) for weapons,
 *      'sub_P_S' (string) for sub-skills.
 * Called once before the render loop — single source of truth.
 */
function buildComputedMap(character, lang) {
  const map = new Map();
  try {
    const rows = getComputedSkills(character, lang, true);
    for (const row of rows) {
      if (row.isCategory) continue;
      map.set(row.globalIndex, {
        rankBonus:    row.rankBonus    ?? 0,
        statBonus:    row.statBonus    ?? 0,
        lvlBonus:     row.lvlBonus     ?? 0,
        miscBonus:    row.miscBonus    ?? 0,
        armorPenalty: row.armorPenalty ?? 0,
        similRanks:   row.similRanks   ?? 0,
        total:        row.total        ?? 0,
        statLabel:    row.statLabel    ?? '',
      });
    }
  } catch (e) {
    console.warn('buildComputedMap error:', e);
  }
  return map;
}

/** Returns computed values for a skill key, with fallback to zeros. */
function getComputed(computedMap, key) {
  return computedMap.get(key) || {
    rankBonus: 0, statBonus: 0, lvlBonus: 0,
    miscBonus: 0, armorPenalty: 0, similRanks: 0, total: 0, statLabel: '',
  };
}
...
function renderSkillsTab(lang) {
  ...
  const isValidated = isGMEditMode() ? false : !!character.phaseValidated;
  ...
  const computedMap = buildComputedMap(character, lang);
  let globalIndex = 0;
      ...
      if (isParent) {
        ...
        if (isWeapon) {
          ...
          for (let ws = 0; ws < character.weaponSkills.length; ws++) {
            ...
            let wTotalRanks = getTotalRanks(character, wsKey);
            // Weapon mastery 3-per-2: chosen weapon gains bonus ranks
            const _bgWpn = getBackgroundBonuses(character);
            if (_bgWpn.weaponMastery && _bgWpn.weaponMastery.mode === '3_per_2'
                && _bgWpn.weaponMastery.weaponIndex === ws) {
            ...
            if (_bgWpn.otherWeaponCostMultiplier !== 1 && _bgWpn.weaponMastery
              ...
              wCost = {
                first: Math.ceil(wCost.first * _bgWpn.otherWeaponCostMultiplier),
                second: wCost.second > 0 ? Math.ceil(wCost.second * _bgWpn.otherWeaponCostMultiplier) : 0,
              };
            }
            ...
            const wc = getComputed(computedMap, wsKey);
            const wRankBonus = wc.rankBonus;
            const wStatBonus = wc.statBonus;
            const wMisc      = character.skillMiscBonuses[wsKey] || 0;
            const wTotal     = wc.total;
            ...
            table += `
              <tr class="${wTotalRanks > 0 ? '' : 'text-gray-600'} ${wHlClass} ${wBoldClass} ${wTextColorClass}">
                ...
                <td class="text-center text-xs col-lvl">${wLvlBonus > 0 ? '+' + wLvlBonus : ''}</td>
                <td class="text-center text-xs col-sim"></td>
        ...
        if (!isWeapon) {
          ...
          for (let si = 0; si < parentSubs.length; si++) {
            ...
            const sc = getComputed(computedMap, subKey);
            const sRankBonus = sc.rankBonus;
            const sStatBonus = sc.statBonus;
            const sStatLabel = sc.statLabel;
            const sMisc = character.skillMiscBonuses[subKey] || 0;
            const sTotal = sc.total;
            ...
            table += `
              <tr class="${sTotalRanks > 0 ? '' : 'text-gray-600'} ${sHlClass} ${sBoldClass} ${sTextColorClass}">
                ...
                <td class="text-center text-xs col-lvl">${sParentLvlBonus > 0 ? '+' + sParentLvlBonus : ''}</td>
                <td class="text-center text-xs col-sim"></td>
        ...
        const c = getComputed(computedMap, globalIndex);
        const rankBonus    = c.rankBonus;
        const statTotalBonus = c.statBonus;
        const lvlBonus     = c.lvlBonus;
        const miscBonus    = c.miscBonus;
        const armorPenalty = c.armorPenalty;
        const simPreview   = isValidated
          ? (character.skillRanksSimil?.[globalIndex] || 0)
          : c.similRanks;
        const total = c.total;
...
function performLevelUp(app) {
  ...
  if (character.devPhase === 'adolescent') {
    ...
    character.level = 1;
    character.bgOptionsLocked = true; // Lock bg options on first real level
    if (character.name) logEvent(character, 'bg_options_locked', {
      phase: 'level', level: 1,
      optionCount: (character.backgroundOptions?.options || []).filter(Boolean).length,
    }, 'Options d\'historique verrouillées (passage niveau 1)');
  } else {
  ...
  const applyAndFinish = (gains) => {
    if (gains) {
      ...
      if (character.name) {
        ...
        logStatGain(character, { level: character.level, gains, summary });
      }
    } else {
    ...
    if (character.name && character.devPhase === 'level') logLevelUp(character, {
...
function bindInfosEvents(app) {
  ...
  if (classEl) {
    classEl.addEventListener('change', () => {
      const newClassIndex = parseInt(classEl.value);
      // Post-apprenti: check prime stats ≥ 90 before allowing class change
      if (character.bgOptionsLocked && !isGMEditMode() && newClassIndex >= 0) {
        const cls = getAllClasses()[newClassIndex];
        const primeIndices = getClassPrimeStats(cls);
        const failing = primeIndices.filter(i => (character.stats[i] || 0) < 90);
        if (failing.length > 0) {
          const names = failing.map(i => (app.lang === 'en' ? STAT_NAMES_EN : STAT_NAMES_FR)[i]).join(', ');
          showToast(app.lang === 'en'
            ? `Cannot adopt this class: prime stats (${names}) must be ≥ 90. Create a new character.`
            : `Impossible d'adopter cette classe : les carac. principales (${names}) doivent être ≥ 90. Créez un nouveau personnage.`, true);
          classEl.value = character.classIndex;
          return;
        }
        // Allowed: update class data but do NOT reapply roll assignments (stats already set)
        character.classIndex = newClassIndex;
        character.realm = getRealmKey(cls);
        character.primeStats = primeIndices;
  ...
  if (raceEl) {
    raceEl.addEventListener('change', () => {
      if (character.bgOptionsLocked && !isGMEditMode()) {
        showToast(app.lang === 'en' ? 'Race is locked after apprenti validation' : 'Race verrouillée après la validation apprenti', true);
        raceEl.value = character.raceIndex;
        return;
      }
...
function bindStatsEvents(app) {
  ...
  if (btnRoll) {
    btnRoll.addEventListener('click', () => {
      ...
      if (isReroll) {
        log.rerollCount++;
        const n = log.rerollCount;
        const msgs = lang === 'en' ? [
          [5,  "Not your lucky day, huh..."],
          [10, "Nudging fate a little... surely this one's good?"],
          [20, "That's a lot of dice... my wrist is killing me!"],
        ] : [
          [5,  "Décidément, c'est pas de chance..."],
          [10, "Il faut bien forcer un peu le destin parfois..."],
          [20, "Ça fait beaucoup de dés lancés... j'ai mal au poignet !"],
        ];
        const hit = msgs.find(([t]) => t === n);
        if (hit) {
          showToast(hit[1]);
        } else if (n > 20 && n % 10 === 0) {
          showToast(lang === 'en'
            ? "Maybe editing the stats directly would be simpler?"
            : "Peut-être qu'éditer directement les caracs serait plus simple ?");
        }
      }
      if (character.name) logStatRoll(character, { method: log.method, rolls: rolledStats.map(r => ({ ...r })) });
  ...
  if (btnValidate) {
    btnValidate.addEventListener('click', () => {
      ...
      statsValidated = true;
      if (character.name) logStatValidate(character, { stats: [...character.stats], potentials: [...character.potentials] });
  ...
  document.querySelectorAll('.stat-input').forEach(input => {
    input.addEventListener('change', () => {
      ...
      // After apprenti: cannot clear a stat to 0 (retirer les carac. interdit)
      if (character.bgOptionsLocked && !isGMEditMode() && val === 0 && (field === 'temp' || field === 'pot')) {
        showToast(app.lang === 'en' ? 'Cannot clear stats after apprenti validation' : 'Impossible de supprimer une caractéristique après la validation apprenti', true);
        input.value = field === 'temp' ? character.stats[idx] : character.potentials[idx];
        return;
      }
...
function bindSpellsEvents(app) {
  ...
  const cls = character.classIndex >= 0 ? getAllClasses()[character.classIndex] : null;
  const classRealm = cls ? getRealmKey(cls) : 'none';
  const studyList = character.spellLists.find(sl => sl.name === study.listName);
  const rankCost = studyList && studyList.type !== 'base_own'
    ? getSpellListCost(cls, classRealm, study.listName, study.listRealm || studyList.reference || '').cost
    : getSpellRankCost(cls);
  ...
  document.querySelectorAll('.spell-remove-list').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (character.name) logSpellListRemove(character, { listName: sl.name });
  ...
  document.querySelectorAll('.add-spell-list:not([disabled])').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const listCost = typeKey === 'base_own' ? getSpellRankCost(cls) : costInfo.cost;
      character.spellLists.push({
        name, maxLevel: 0, reference: realm, type: typeKey, realm: realm, dpPerRank: listCost,
      });
      character.spellLog.push({
        timestamp: new Date().toISOString(), action: 'add', listName: name,
        phase: character.devPhase, details: { realm, type: typeKey },
      });
      if (character.name) logSpellListAdd(character, { listName: name, listType: typeKey, realm });
      renderEditor(app);
    });
  });
  ...
  function finalizeSpellLearn() {
    ...
    // Log spell learn event
    if (character.name) logSpellLearn(character, {
      listName: study.listName, spellLevel: blockEnd,
      blockStart, blockEnd, type: study.listType, realm: study.listRealm,
    });
    // Reset study
    study.listName = null; study.listType = null; study.listRealm = null;
    study.ranks = 0; study.sgrDone = false; study.blockSize = 5; study.nextBlockStart = 1;
...
function bindHistoryEvents(app) {
  ...
  document.querySelectorAll('.bg-opt-roll').forEach(btn => {
    btn.addEventListener('click', () => {
      if (character.bgOptionsLocked && !isGMEditMode()) {
        showToast(lang === 'en' ? 'Background options locked after apprenti validation' : 'Options d\'historique verrouillées après la validation apprenti', true);
        return;
      }
  ...
  // Remove background option (with rollback and logging)
  document.querySelectorAll('.bg-opt-remove').forEach(btn => {
    btn.addEventListener('click', () => {
      const idx = parseInt(btn.dataset.bgIdx);
      const opt = character.backgroundOptions?.options?.[idx];
      const result = removeBackgroundOption(character, idx);
      if (!result.success) { showToast(result.reason || 'Cannot remove'); return; }
      if (character.name) logBgOptionRemove(character, {
        optionName: opt?.name_fr || opt?.name || '?',
        rollbackSummary: result.rollbacks.join(', ') || 'none',
      });
      renderEditor(app);
    });
  });
  ...
  // Inline bg choice dropdowns / text inputs
  document.querySelectorAll('.bg-choice-select').forEach(sel => {
    sel.addEventListener('change', () => {
      const idx = parseInt(sel.dataset.opt);
      const type = sel.dataset.type;
      if (!sel.value) return;
      const opt = character.backgroundOptions?.options?.[idx];
      // skill_ranks_target stores a numeric globalIndex, not a string
      const val = type === 'skill_ranks_target' ? parseInt(sel.value) : sel.value;
      resolveBackgroundChoice(character, idx, { [type]: val });
      if (character.name && opt) logBgOptionResolve(character, {
        optionName: opt.name_fr || opt.name || '?',
        choiceType: type, choiceLabel: sel.options[sel.selectedIndex]?.text || val,
      });
      renderEditor(app);
    });
  });

  document.querySelectorAll('.bg-cost-target-select').forEach(sel => {
    sel.addEventListener('change', () => {
      const idx = parseInt(sel.dataset.opt);
      ...
      const targets = [...(opt.resolved.skill_cost_targets || [])];
      targets[slot] = parseInt(sel.value);
      opt.resolved.skill_cost_targets = targets;
      renderEditor(app);
    });
  });
  document.querySelectorAll('.bg-choice-input').forEach(input => {
    input.addEventListener('change', () => {
      const idx = parseInt(input.dataset.opt);
      const type = input.dataset.type;
      if (input.value.trim()) { resolveBackgroundChoice(character, idx, { [type]: input.value.trim() }); renderEditor(app); }
    });
  });

  // Background option resolve buttons (fallback for choice types without inline UI)
  ...
  // Wire filter chips for journal
  const filterChips = document.querySelectorAll('.rm-filter-chip[data-filter]');
  const journalContainer = document.getElementById('event-journal-container');
  filterChips.forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      if (journalContainer) {
        journalContainer.querySelectorAll('.rm-timeline-item').forEach(item => {
  ...
  // Migration: if character.eventLog is empty but IndexedDB has history, back-fill it
  if (character.name && (!character.eventLog || character.eventLog.length === 0)) {
    getCharacterHistory(character.name, { limit: 200 }).then(history => {
      if (!history || history.length === 0) return;
      character.eventLog = history.map(e => ({
        timestamp: e.timestamp, type: e.type, data: e.data,
        summary: e.summary, phase: e.phase, level: e.level,
      }));
      const container = document.getElementById('event-journal-container');
      if (container) container.innerHTML = renderEventLogHTML(character.eventLog, lang);
    }).catch(() => {});
  }
}
...
function bindSkillsEvents(app) {
  ...
  if (btnValidatePhase) {
    btnValidatePhase.addEventListener('click', () => {
      ...
      if (character.name) logPhaseValidate(character, {
        phase: character.devPhase === 'level' ? character.level : character.devPhase,
  ...
  // Auto-assign DP button
  const btnAutoAssign = document.getElementById('btn-auto-assign-dp');
  if (btnAutoAssign) {
    btnAutoAssign.addEventListener('click', () => {
      const cls = character.classIndex >= 0 ? getAllClasses()[character.classIndex] : null;
      const archetype = detectArchetype(cls);
      ...
      if (!hasWeapons) {
        ...
        if ((character.weaponSkills || []).length === 0) {
          ...
          if (!character.skillRanksSimil) character.skillRanksSimil = {};
          for (let wi = 0; wi < (character.weaponSkills || []).length; wi++) {
            character.skillRanksSimil['wpn_' + wi] = calcWeaponSimilarityRanks(wi, character);
          }
        }
  ...
  document.querySelectorAll('.skill-misc-input').forEach(input => {
    input.addEventListener('change', () => {
      ...
      const oldVal = character.skillMiscBonuses[idx] || 0;
      const val = parseInt(input.value) || 0;
      if (val) {
      ...
      // Log manual edit
      if (character.name && val !== oldVal) {
        const _skills = getAllSkillsFlat();
        const skillName = typeof idx === 'number' && _skills[idx] ? (_skills[idx].name_fr || _skills[idx].name) : String(idx);
...
function openPrintConfigPopup() {
    ...
    <div class="bg-gray-800 rounded-lg p-4 overflow-y-auto" style="display:flex;flex-direction:column;width:min(100%,32rem);max-height:90vh">
      ...
      <div class="grid gap-3 text-sm" style="grid-template-columns: minmax(auto,max-content) 1fr">
        ...
        <div class="flex flex-col gap-1">
          ...
          <label class="text-xs cursor-pointer" style="color:#f59e0b"><input type="checkbox" id="pc-blank" ${pc.blankSheet ? 'checked' : ''}> ${lang === 'en' ? 'Blank sheet (all fields empty)' : 'Feuille vierge (tous les champs vides)'}</label>
        </div>
  ...
  document.getElementById('pc-print').addEventListener('click', () => {
    ...
    pc.blankSheet = document.getElementById('pc-blank').checked;
