...
import { saveCharacter, loadCharacter, deleteCharacter,
         getAllCharacters, migrateFromLocalStorage, appendEvent } from './db.js';
 ...
 * Also seeds IndexedDB with any embedded eventLog entries (async, fire-and-forget).
 */
export function characterFromJSON(jsonStr) {
  ...
  // Seed IndexedDB from embedded log if present
  if (Array.isArray(character.eventLog) && character.eventLog.length > 0) {
    for (const entry of character.eventLog) {
      appendEvent({ characterName: character.name, ...entry }).catch(() => {});
    }
  }
  return character;
}
