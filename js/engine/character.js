...
export function createCharacter() {
  return {
    ...
    // Event log — portable journal embedded in save
    eventLog: [],       // [{timestamp, type, data, summary, phase, level}]

    // Background options lock (set true on first level-up)
    bgOptionsLocked: false,
...
export function getTotalRanks(character, skillIndex) {
  ...
  const bgRanks = (typeof skillIndex === 'number' && bgBonuses.bgSkillRanks[skillIndex]) || 0;
...
export function getTotalStatBonus(character, statIndex) {
  ...
  const bgBonuses = getBackgroundBonuses(character);
  const bg = bgBonuses.statBonusMods[statIndex] || 0;
  return normal + race + special + bg;
}
