...
import { calcHitPoints, calcPowerPoints, calculateDB,
         getDeathThreshold } from './character.js';
import { getBackgroundBonuses, summarizeBackgroundBonuses } from './background-effects.js';
...
function drawSkillRow(doc, sk, x, y) {
  ...
  const isSub   = sk.isSubSkill || sk.isWeapon || sk.name.startsWith('  ');
  const fStyle  = sk.bold ? 'bold' : 'normal';
  doc.setFont('helvetica', fStyle);
  doc.setFontSize(isSub ? 7 : 7.5);

  // Strip '↳ ' prefix — jsPDF Helvetica is Latin-1 only, the arrow measures as
  // 0-width but renders as a replacement glyph, causing overflow into DM boxes.
  // The 2mm indent already conveys sub-skill nesting.
  let name = sk.name.trim().replace(/^↳\s*/, '');
  const maxNameW = isSub ? 34 : 38;
  while (name.length > 0 && doc.getTextWidth(name) > maxNameW) name = name.slice(0, -1);
  doc.text(name, x + (isSub ? 2 : 0), y);
...
export function generateCharacterPDF(character, options = {}) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const total = bonus + race + bg + spec;
  ...
  // ==============================
  const developedSkills = getComputedSkills(character, lang)
    .filter(s => s.isCategory || s.totalRanks > 0 || s.total > 0)
    .map(s => options.showStats && s.statLabel
      ? { ...s, name: s.name + ` (${s.statLabel})` }
      : s);
